/***************************
  Challenge: To calculate the total number of stairs climbed in 20 years;
  
  parameter is an array of arrays;
  return a single number value;
  taking the test case as an example - 
        ...
        stairsIn20(stairs), 54636040;
        
    to solve this, my attempt is to call the reduce method on the parent array to see if it would work,
        if it doesn't, then we call the reduce method on each child array, multiply by 20 to obtain the total for 20 years and then the parent array to obtain the grand total of all child summations;
***************************/

function stairsIn20(s){
  //your code here
  const sumOfEachChild = s.map(e => e.reduce((acc,c) => +acc + +c) * 20);
  
  return sumOfEachChild.reduce((acc,c) => +acc + +c);
}